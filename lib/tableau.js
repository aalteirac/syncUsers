...
async function getUsersPage(token,siteid,pagesize=100,pagenum=1){
    return new Promise((resolve, reject) => {
        get(`${TABURL}/sites/${siteid}/users?pageSize=${pagesize}&pageNumber=${pagenum}`,)
        .header("Content-Type", "application/json")
        .header("Accept", "application/json")
...
async function updateUser(token,siteid,user){
    return new Promise((resolve, reject) => {
        ...
        .then((response) => {
            if(response.error){
                if(response.body.error.code=='403009'){
                    //TOL BUG
                    resolve(response.body);
                }
                else{
...
async function _addUser(token,siteid,user){
    ...
    if(res.error) {
        if(res.error.code=='403009'){
            //TOL BUG raisning a 403 when it's actually done right...
